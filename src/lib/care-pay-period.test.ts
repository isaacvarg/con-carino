...
describe('payOverviewRange — weekly', () => {
  it('starts on the Sunday of the current week and ends the next Sunday', () => {
    const r = payOverviewRange('WEEKLY', 0, tuesday)
  ...
  it('treats a Sunday as the first day of its own week', () => {
    ...
    expect(ymd(payOverviewRange('WEEKLY', 0, sunday).start)).toBe('2026-07-26')
  })

  it('steps backwards and forwards by whole weeks', () => {
    expect(ymd(payOverviewRange('WEEKLY', -1, tuesday).start)).toBe('2026-07-19')
    expect(ymd(payOverviewRange('WEEKLY', 1, tuesday).start)).toBe('2026-08-02')
  })

  it('crosses a year boundary going back', () => {
    const jan = new Date(2026, 0, 2, 12) // Friday 2026-01-02
    const r = payOverviewRange('WEEKLY', -1, jan)
  ...
  it('always spans exactly seven days', () => {
    for (const offset of [-9, -1, 0, 1, 5, 30]) {
      const r = payOverviewRange('WEEKLY', offset, tuesday)
...
describe('payOverviewRange — monthly', () => {
  it('covers the calendar month containing today', () => {
    const r = payOverviewRange('MONTHLY', 0, tuesday)
    expect(ymd(r.start)).toBe('2026-07-01')
    expect(ymd(r.end)).toBe('2026-08-01')
  })

  it('steps by whole months', () => {
    expect(ymd(payOverviewRange('MONTHLY', -1, tuesday).start)).toBe(
      '2026-06-01',
    )
    expect(ymd(payOverviewRange('MONTHLY', 1, tuesday).start)).toBe('2026-08-01')
  })

  it('does not skip February when stepping back from a 31st', () => {
    const march31 = new Date(2026, 2, 31, 8)
    const r = payOverviewRange('MONTHLY', -1, march31)
  ...
  it('handles February in a leap year', () => {
    ...
    const r = payOverviewRange('MONTHLY', 0, feb)
  ...
  it('crosses a year boundary in both directions', () => {
    ...
    expect(ymd(payOverviewRange('MONTHLY', -1, jan).start)).toBe('2025-12-01')
    const dec = new Date(2026, 11, 15, 8)
    expect(ymd(payOverviewRange('MONTHLY', 1, dec).start)).toBe('2027-01-01')
  })
})
...
describe('payOverviewLabel', () => {
  ...
  it('adds the year only when it differs from today', () => {
    const r = payOverviewRange('MONTHLY', -7, tuesday)
